...
do -- automatically select last venv
  if LazyVim.has("venv-selector.nvim") then
    local function update_venv()
      ...
      local orig = venv_selector.venv()
      cache.retrieve()

      -- The language server may still be starting, and cache.retrieve() schedules
      -- setting the venv.
    ...
    LazyVim.on_load("venv-selector.nvim", update_venv)
  end
end

do -- lazily install tools
