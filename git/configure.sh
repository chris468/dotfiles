...
# ARG_HELP([Configures the git configuration])
# ARGBASH_GO()
...
print_help()
{
	printf '%s\n' "Configures the git configuration"
...
excludesfile_destination=~/.config/git/ignore

if [ -e "$excludesfile_destination" ] ; then
    if [ "$_arg_quiet" == "on" ] ; then
        exit 0
    fi

    if [ "$_arg_force" != "on" ] ; then
        die "$excludesfile_destination already exists"
    fi
fi

rm -f $excludesfile_destination
mkdir -p $(dirname "$excludesfile_destination")
ln -s $script_dir/ignore $excludesfile_destination
