...
local map, ext, temporarily_disable_relativenumber =
    util.map, util.ext, util.temporarily_disable_relativenumber
...
local mappings = {
    ...
    -- Browsers
    { map = "<leader>ef", cmd = ext("nvim-tree.api", function(t)
        t.tree.open()
    end) },
    {
        map = "<leader>eF",
        cmd = ext("nvim-tree.api", function(t)
            t.tree.open({ find_file = true })
        end),
    },
    { map = "<leader>et", cmd = ext("nvim-tree.api", function(t)
        t.tree.toggle()
    end) },
    ...
    {
        map = "<leader>rt",
        cmd = ext("telescope.builtin", function(telescope)
            telescope.resume()
        end),
    },
...
local function dap_mappings(dap)
    local debugger = require("chris468.dap.launch")
    return {
        {
            map = "<leader>dg",
            cmd = function()
                debugger.launch_or_continue(dap)
            end,
        },
        {
            map = "<leader>dG",
            cmd = function()
                debugger.prompt_or_continue(dap)
            end,
        },
...
map(mappings)
if_ext("dap", function(dap)
    map(dap_mappings(dap))
end)
